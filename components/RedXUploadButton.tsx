...
interface RedXUploadButtonProps {
  isVerified: boolean;
  soundId?: string; // If uploading to a specific sound
  onUploadComplete?: () => void;
}

export default function RedXUploadButton({ isVerified, soundId, onUploadComplete }: RedXUploadButtonProps) {
  ...
  // Open the camera interface with vertical split animation
  const handleButtonPress = useCallback((e: React.MouseEvent | React.TouchEvent) => {
    ...
    const handleRelease = () => {
      ...
      if (pressDuration < 200) {
        // Quick tap - Photo mode
        setMode('photo');
      } else {
        // Long press - Video mode
        setMode('video');
      }
  ...
  // Check if Elite 6 is full for this sound
  const checkElite6Status = async () => {
    ...
    try {
      const { data, error } = await supabase
        .from('elite_6_videos')
        .select('id, video_url, thumbnail_url, quality_score, slot_number, creator_id')
        .eq('sound_id', soundId)
  ...
  const handleElite6Swap = async (videoToReplace: Elite6Video) => {
    ...
    try {
      ...
      // Replace the video using RPC function
      const { error: replaceError } = await supabase.rpc('replace_elite_6_video', {
        ...
        p_new_quality_score: 50 // Default quality score
      });
  ...
  return (
    <>
      ...
      >
        ...
        {/* Glow effect when Elite 6 is full */}
        {showElite6Swap && (
          <motion.div
                  ...
                  <div className="absolute bottom-8 left-0 right-0 flex justify-center items-center gap-8">
                    ...
                    {mode === 'video' && isRecording && (
                      <button
                        ...
                        className="p-6 bg-red-600 rounded-full shadow-lg relative"
                      >
                ...
                <div className="flex-1 flex flex-col bg-black">
                  ...
                  <div className="p-4 flex justify-center gap-4">
                    <button
                      onClick={handlePost}
                      disabled={loading}
                      className="px-8 py-3 bg-gradient-to-r from-red-600 to-red-500 rounded-full text-white font-bold disabled:opacity-50"
                    >
